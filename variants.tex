...
$(q,\updinstr{d\vr v}, q')$.
Furthermore, to correctly simulate a zero-test of counter $j$, 
say a transition $(q,\ztinstr j,q')$,
we replace the vectors $(1,-1)$ and $(-1,1)$ in the first
and last transition of \eqref{eq:sim3} by
\[
(d,\ldots,d,-(j-1), d, \ldots, d)
\qquad\text{ and } \qquad
(-d,\ldots,-d,j-1, -d, \ldots, -d),
\]
respectively, where the value $j-1$, respectively~$-(j-1)$, appears on 
the $j$th coordinate.
...
Indeed, 
whenever a symmetric \cm has a run $q(0,0)\step^* q'(0,0)$, it also has a
\emph{simple} run, in which all counter updates are performed on
counter 1, while all zero-tests are (trivially) performed on counter 2.
