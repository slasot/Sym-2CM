...
\para{Terminology}
We use the triple exponential function $\exptriple(m) = 2^{2^{2^m}}$.
...
and $\norm X$ is bounded (triply) exponentially in $n$:
$\norm X \leq 2^{\poly n}$, resp.~$\norm X \leq \exptriple(\poly n)$.
...
\emph{triply exponentially bounded} if
  ...
  ${\norm S}^{O(1)}\cdot \exptriple(\poly n)$,
...
%  there is $d \in \N$ and polynomial $p$ such that 
 if a new base appears in $X_{i+1}$ then
 \[
    \norm{X_{i+1}} \leq \norm{X_i}^{O(1)} \cdot \exptriple(\poly n),
 \]
...
have 
\[
\norm{X_i} \leq \norm{X_0}^{O(\mysize Q \cdot D)} \cdot \exptriple(\poly n),
\]
...
\begin{claim}\label{claim:sl4}
  ...
  is triply exponentially bounded and computable in
...
Now, a key observation is that once $Y$ is nonempty and the set
...
To summarize, the running time of our decision procedure is at
