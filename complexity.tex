...
and we will be content with a 6-exponential bound.
...
% x}
is polynomial in $n$.
The norm of the effect of every cycle 
% of length at most $e_\text{max}$ 
...
of the integer linear programming problem of size polynomial in $\ell$, 
...
The period $N^c_{q,q'}$ of ultimately periodic presentation of $X^c_{q,q'}$ 
is at most doubly exponential, as the least common multiplicity of periods
of its base-period representation.
So is the least common multiple $N$ of
all the periods $N^c_{q,q'}$, across all choices of $c, q$ and $q'$.
By \Cref{claim:sl}, the bases of $N^c_{q,q'}$ are bounded by $N$ as well.
...
\begin{claim}\label{claim:sl4}
  Given a k-exponential semilinear set $S$, $k\geq 3$, the set
    $\Phi_3 \circ S$ is also
...
We start by analysing the computation of $X_0 = f(\{s\})$.
By \Cref{claim:sl} the set $(\stepp + \circ \ X_{q,r})$
is exponentially semilinear.
Then by \Cref{claim:sl4} applied to $S= (\stepp + \circ \ X_{q,r})$, the 
% $D$-saturated 
set 
...
Combining this with an exponential blowup incurred by a necessity to generate
