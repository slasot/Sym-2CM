...
\[
\norm{X_i} \leq \bigl(\norm{X_0} \cdot 
\expf 3(\poly n)\bigr)^{O(\mysize Q \cdot D)},
\]
...
\begin{claim}\label{claim:sl4}
  Given a 3-exponential semilinear set $S$, the set
    $\Phi_3 \circ S$ is also
  3-exponentially semilinear and computable in
...
$T\mapsto T \cup T^2$, each self-composition increasing the norm 
...
on the computation time for $X_0$.
...
The running time of our decision procedure is hence elementary.
% 
