...
\[
\mysize \mach = 
\mysize Q + 2 \cdot \norm \mach \cdot \mysize {T_\mathrm{upd}}
\ + \ \norm w \ + \ \norm{w'},
\]
\mei{wait, do we want to include $w$ and $w'$ within
$\mysize \mach$? Doesn't feel right...}
where $T_\mathrm{upd}$ is the set of update transitions of $\mach$.
...
For a semilinear set $X$ 
with some given base-period representation
...
This, combined with the exponential blow-up of Parikh's theorem, \lukasz{Should we add citation?}
...
\]
(relying on \Cref{claim:sl2} and \Cref{claim:cap})
...
the set 
\[ Y \ \circ \ (\stepp - \, \circ \, \stepp{0}^*)^{\leq m}
\]
