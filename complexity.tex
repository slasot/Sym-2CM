...
\section{Complexity bound for \Cref{thm:main}} \label{sec:complexity}
...
as the size of the \emph{unary}
representation of a 2-\cm $\mach=(Q,T)$, the source $s=q(\vr w)$ 
and the target $t=q'(\vr w')$, namely
\[
% \mysize \mach = 
n \, = \,
\mysize Q + \mysize{T_\text{zero-test}} \, + \, 
2 \cdot \norm \V \cdot \mysize {T_\text{update}}
\, + \, \norm{\vr w} \, + \, \norm{\vr w'},
\]
% \mei{wait, do we want to include $w$ and $w'$ within
% $\mysize \mach$? Doesn't feel right...}
% \sla{i think we should. I've removed the notation $\mysize \mach$, let's 
% simply use $n$ instead}
where $T_\text{zero-test}$ and $T_\text{update}$ are the sets of zero-test
and update transitions of $\mach$, respectively, and 
$\V=(Q,T_\text{update})$ is the underlying 2-\vass.
...
in terms of $n$, rather than
optimizing the number of exponents, 
and we will be content with a 5-exponential bound.
...
% x}
is polynomial in $n$.
The norm of the effect of every cycle of length at most $e_\text{max}$ 
is bounded by $e_\text{max}\norm{\V}$,
where $e_\mathrm{max}$ is the maximal length $e$ (polynomial in $n$),
and hence
...
\para{Terminology}
We use the $k$-exponential function $\expf k$, defined as
$\expf 0 (m) = m$, and $\expf {k+1} (m) = 2^{\expf k (m)}$.
%  (m) = 2^{2^{2^m}}$.
Whenever a base-period representation of a semilinear set $X$ is clear
from the context, 
% \sla{reprezentacji moze byc oczywiscie wiele, mam nadzieje ze to jest
% zrozumiale}, 
we write
$\norm{X}$ for the maximum of norms of bases
and periods of this representation.
In the sequel we say that a semilinear set $X$ is 
\emph{$k$-exponentially semilinear} if given $\mach$,
one can compute
a base-period representation of $X$
of norm bounded by $\norm X \leq \expf k(\poly n)$.
% in time $\norm X \leq \expf k(\poly n)$, and 
...
\end{align}
where 
$\ell$ is polynomial in $n$, and
$t_0, \ldots, t_\ell$ are paths and $c_1, \ldots, c_\ell$ are cycles of
length polynomial in $n$ (cf.~\eqref{eq:lps}).
...
is the union of (the projections of) the solution sets of instances $I$ 
of the integer linear programming problem of size polynomial in $n$, 
...
whose bases and periods are of norm at most $C+1$, the constant of
\Cref{lem:targetpump}
...
$\stepp 0$,
we conclude that sets $X^c_{q,q'}$ %\eqref{eq:per1},
 \eqref{eq:per2} 
in the proof of \Cref{lem:0star} have 
base-period representations with at most exponential periods and bases.
The period $N^c_{q,q'}$ and threshold $M^c_{q,q'}$ 
of ultimately periodic presentation of $X^c_{q,q'}$ 
are at most doubly exponential, as they are bounded by 
the least common multiplicity of  bases and periods
of its base-period representation.
So is the least common multiple 
$N$ of all periods of $N^c_{q,q'}$,
across all choices of $c, q$ and $q'$, which is doubly exponential.
% By \Cref{claim:sl}, the bases of $X^c_{q,q'}$ are bounded by $N$ as well.
...
\cite[Thm.~11]{KopczynskiTo:LICS2010},
...
\begin{claim} \label{claim:sl2}
  The relation $\stepp{0}^*$ is 3-exponentially semilinear.
...
$\norm{r_1 \circ r_2} \leq (O(1)+\norm{r_1} \cdot \norm{r_2})^{O(1)}$.
\end{claim}
Indeed, ignoring $Q$ for simplicity, \Cref{claim:circ} 
...
\]
Likewise we get the bound for images and inverse images of relations:
\begin{claim} \label{claim:im}
Semilinear relations $r\subseteq (Q\times\N)^2$ and semilinear sets
$X\subseteq Q\times\N$ satisfy
$\norm{r \circ X}, \norm{X \circ r} \leq 
(O(1)+\norm{r} \cdot \norm{X})^{O(1)}$.
\end{claim}


% Using Claims \ref{claim:sl}--\ref{claim:circ} we conclude
% that the relation
%   $(\stepp - \circ \, \stepp{0}^*)$ is triply exponentially semilinear,
% which allows us to deduce further bounds on bases and periods
...
% For later use we state a bound on the bases and periods 
% of the semilinear image of $(\stepp - \circ \, \stepp{0}^*)$:

% \begin{claim}\label{claim:sl3a}
%     The mapping
%   $S\mapsto S \,\circ\,(\stepp - \circ \, \stepp{0}^*)$
%   is triply exponentially bounded.
% \end{claim}
...
Due to Claims \ref{claim:sl2} and \ref{claim:im}, 
...
\emph{newly created}, namely
 ...
 Again due to Claims \ref{claim:sl2} and \ref{claim:im},
%  \begin{claim}\label{claim:sl3m}
% The mapping $S \mapsto (\stepp - \circ \, \stepp{0}^*)^m \,\circ\, S$
%   is triply exponentially bounded.
% \end{claim}
 if a new base appears in $X_{i+1}$ then
 \[
    \norm{X_{i+1}} \leq \norm{X_i}^{O(1)} \cdot \expf 3(\poly n),
 \]
...
have 
\begin{align} \label{eq:3e}
\norm{X_i} \leq \bigl(\norm{X_0} \cdot 
\expf 3(\poly n)\bigr)^{2^{O(\mysize Q \cdot D)}},
\end{align}
% \sla{to nie jest w formacie triply expoentially bounded}
...
\begin{claim}\label{claim:sl4}
  Given a $k$-exponentially semilinear set $S$, $k\geq 3$, 
  the set $\Phi_3 \circ S$ is 
  $k$-exponentially semilinear and computable in
  at most $k$-exponentially many iterations of \eqref{eq:tra}.
\end{claim}
...
We start by analyzing the computation of $X_0 = f(\{s\})$.
By \Cref{claim:sl} the set $(\stepp + \circ \ X_{q,r})$
is exponentially semilinear.
Then by \Cref{claim:sl4} 
applied to $k=3$ and 
$S= (\stepp + \circ \ X_{q,r})$, the set 
$S'=\Phi_3 \,\circ\, ({\stepp +}\,\circ \,X_{q,r}) = 
(\Phi_3 \, \circ \, {\stepp +})\,\circ \,X_{q,r}$
is computable in at most triply exponentially many iterations
of the transformation \eqref{eq:tra}, say $m$ iterations.
Then intersection $Y$ of $S'$
...
\]
has
(relying on Claims \ref{claim:sl2} and \ref{claim:cap})
bases and periods of at most triply exponential norm. 
Assume, \mywlog, that $m=2^\ell$ for some doubly exponential $\ell$, 
an let $R = ({\stepp -} \, \circ \, {\stepp{0}^*})^{\leq m}$.
The relation $R$ is computable by $\ell$-fold self-composition
$T\mapsto T \cup T^2$ starting from $T=({\stepp -} \, \circ \, {\stepp{0}^*}) \cup \text{Id}$,
each self-composition increasing the norm 
polynomially, which implies:
\begin{claim} \label{claim:sl5}
  The relation $R$ is 4-exponentially semilinear.
%   and the norms of bases and periods of its semilinear description
%   are bounded triply-exponentially in $n$.
\end{claim}
% 
We have $\Phi_3 \circ ({\stepp +}\circ X_{q,r}) =
R \circ ({\stepp +}\circ X_{q,r})$.
Now, a key observation is that once $Y$ is nonempty,
there is a pair $(y,y')\in Y\times ({\stepp +} \circ X_{q,r})$
such that
$ (y,y') \in R$,
...
the intersection contains, due to Claims \ref{claim:sl}, \ref{claim:circ}, 
\ref{claim:im} and \ref{claim:sl5}, 
a number
which is at most 4-exponential in $n$.
% \sla{nie, wzrost jest wiekszy niz triply exponential; trzeba wzmocnić 
% \Cref{claim:sl3a}}
This provides a 4-exponential upper bound on the number of iterations 
of the exhaustive search 
in the proof of \Cref{lem:compbackfor}, and thus also the same
% , relying on \Cref{claim:sl4}, the 
bound on $\norm{X_0}$.
Testing candidates in the exhaustive search requires only
4-exponentially many iterations of \eqref{eq:tra}, due to
\Cref{claim:sl4} (applied to $k=4$).

When computing the iteration $S \mapsto S \cup f(S)$,
we only apply \Cref{claim:sl4} to sets of the form
${\stepp +} \circ X_{q,r}$, which do not themselves depend
on the input set $S$. As such,
the bound $m$ on the number of iterations does not depend
on the input set $S$ either,
and in the same way as above
we obtain the same 4-exponential bound on the norm of every
subsequent iterate $X_{i+1}=X_i \cup f(X_i)$.

Finally, the 4-exponential bound on the number of iterations 
\eqref{eq:tra2} before a fixpoint is reached in shown similarly
as in the first part.
Similarly as in \eqref{eq:3e} we have
\[
\norm{X_i} \leq \bigl(\norm{X_0} \cdot 
\expf 4(\poly n)\bigr)^{2^{O(\mysize Q \cdot D)}},
\]
which yields a 4-exponential bound on $\norm{X_i}$.
...
To summarize, we have obtained the bounds on the norms of 
representations of computed semilinear sets, and on the numbers
of iterations performed in the course of computation.
The bounds are 4-exponential in $n$, the size of unary representation,
and hence at 5-exponential in the size of binary representation.
Combined with an exponential blowup incurred by a necessity to generate
all bases and periods, this yields
the elementary running time of our decision procedure.
% 
As the only known lower bound is \pspace-hardness
