...
Relying on Remark \ref{rem:wlog}, our task is to decide if 
$s\reachrel^* t$
for $s,t\in Q\times\N$,
...
Thus, the reachability relation is 
\[
\reachrel^* \ = \ 
...
as well as
the image of relation $\Phi_1$ and inverse image of $\Phi_3$, are
...
semilinear sets it maintains form
...
\begin{proof}[Proof of \Cref{lem:0star}]
   ...
   (doable similarly, by a decrement, zero-test, and increment).
    ...
    in states), 
    add anothers $N$ state $p'_{N-1} \ldots p'_{0}$ forming 
...
\]
is effectively semilinear and $D$-\saturated, as well as every next iterate
\[
X_{i+1} \ := \ X_{i} \ \cup \ (\stepp - \circ \stepp {0}^\ast) \circ X_{i},
\]
for $0 = 1, 2, \ldots$. 
The sequence $X_0, X_1, \ldots$ is a 
non-decreasing
chain of $D$-\saturated sets and hence stabilises, by \Cref{lem:stab}, namely
\[
...
\{t\} \ \cup \ X_j
\]
is effectively semilinear, as union of $\{t\}$ and a $D$-\saturated set.
