...
with resricted update instructions but unrestricted zero-test instructions.
% 
% \begin{remark}
...
setting. We leave for future investigations to establish the 
expressive power of the fragmet, and the complexity of 
...
and the last \Cref{sec:complexity} discusses complexity bounds.
...
whose reminder modulo $N$ equals $r$ and  $(q,n)\in X$ 
...
which implies that so is $\reachrel$, the restriction (section) thereof 
enforcing a counter to be 0 initially or finally, 
and also further Presburger-definable restrictions 
$\stepp 0, \ \stepp -, \ \stepp +$.
\end{proof}
...
for every positively verified state-reminder pair $(q,r)$,
