...
\begin{equation} \label{eq:steep-ind}
    ...
    \qquad\qquad
    \sla{Y_i - S^y_i \, \leq \, a \, + \, \beta_i (X_i-S^x_i)}
\end{equation}
\mei{TODO Figure.}
\sla{on the right: porposed invariant; $(S^x_i,S^y_i)$ is the one-off effect}
...
\begin{align*}
    y - Y_i &\ \leq \ S_{i+1} - S_i,     & \sla{y - Y_i \ = \ S^y_{i+1} - S^y_i}, \\
    X_i - x &\ \leq \ S_{i+1} - S_i. & \sla{x - X_i \ = \ S^x_{i+1} - S^x_i}
\end{align*}
Multiplying the latter by $\beta_i$ and rearranging, \mei{chuj dupa to nie działa dla $\beta_0 = -1$} we obtain
\[
    y - Y_i \ \leq \ (S_{i+1} - S_i)(1 + \beta_i) \, + \, \beta_i (x - X_i),
    \qquad\qquad\sla{niepotrzebne}
\]
...
\begin{equation} \label{eq:steep-interm}
    \begin{split}
        ...
        & \sla{y - S^y_{i+1} \, \leq \, a \, + \, \beta_i (x-S^x_{i+1})} \\
...
\[
    ...
    &\sla{Y_{i+1} - S^y_{i+1} \, \leq \, a \, + \, \beta_i (X_{i+1}-S^x_{i+1})}
    \end{split}
\]
...
\emph{(triply) exponentially based} if
  ...
  $S\subseteq Q\times\N$ where all numbers are bounded by $m$, 
