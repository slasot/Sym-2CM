...
\begin{tikzpicture}[
  ...
  \node at (5.1,2.0) {$\varphi_1>\varphi_2>\varphi_3$};

  % labels
  \node[below right] at (4.3,0.27) {horizontal effect};
  \node[above left] at (-0.2,2.07) {vertical effect};

  % sign labels
  \node at (2.1,-0.3) {$x_1,x_2>0$};
  \node at (-2.0,-0.3) {$x_3<0$};
\end{tikzpicture}
\caption{
  The steepness comparison used in
  \Cref{claim:steep}.
  The vectors represent the effects of three cycles
  $c_1$, $c_2$, and $c_3$, where $c_1$ is horizontally
...
\node[below] at (P3e) {$(X_3, Y_3)$};
  ...
  % angle / steepness indication
  \node at (10.1,1.0) {$\varphi_1<\varphi_2<\varphi_3$};

\end{tikzpicture}

\caption{A run violating Claim~13.  
Thick lines represent iterated major cycles, and dotted lines represent 
one-off paths.
The cycles $c_1$ and $c_2$ are 
(horizontally positive) major sum-increasing cycles, the latter steeper than
the former, and $c_3$ is a horizontally negative major (sum-decreasing) cycle
steeper than $c_1$ and $c_2$.
Thus none of major sum-increasing§ cycles is followed by a steeper
