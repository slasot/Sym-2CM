...
the sufficiently large value of $C$ is $C := C_1 (1 + e\norm\V)$.
...
Moroever, numbers appearing in $I$ are bounded exponentially in
...
the reachability relation $\step^*$ of $\V$ is effectively semilinear, 
...
a section of the reachability relation of $\V$.
...
This, combined with the exponential bound of Parikh's theorem,
...
increases the norms of bases and periods at most polynomially.
Using \Cref{claim:sl2} we conclude
that the relation
  $(\stepp - \circ \, \stepp{0}^*)$ is effectively semilinear,
  and the norms of bases and periods of its semilinear description
  are bounded triply-exponentially in $n$.
This allows us further to deduce:

\begin{claim}\label{claim:sl3}
  Given a semilinear ($D$-saturated) set $S\subseteq Q\times\N$, 
  the bases of the $D$-saturated
  set  $(\stepp - \circ \, \stepp{0}^*) \,\circ\, S$
  are larger than the bases of $S$ by at most
  triply exponential multiplicative factor.
\end{claim}

For future use we also state a dual bound:

\begin{claim}\label{claim:sl3a}
  Given a semilinear set $S\subseteq Q\times\N$, 
  the bases of the semilinear
  set  $S \,\circ\,(\stepp - \circ \, \stepp{0}^*)$
  are larger than the bases of $S$ by at most
  triply exponential multiplicative factor.
\end{claim}
...
\emph{newly created}, namely
 ...
 times (exponentially many times) during the whole iteration, and is
 ...
 yields still a triple exponential value,  
...
\begin{claim}\label{claim:sl4}
  Given a semilinear ($D$-saturated) set $S\subseteq Q\times\N$, 
  the bases of the $D$-saturated
  set  $\Phi_3 \circ S$, as well as the number of iterations needed
  to compute it,
  are larger than the bases of $S$ by at most
  triply exponential multiplicative factor.
\end{claim}
...
of semilinear sets in fixed dimension)
...
This provides a triply exponential bound on the exhaustive search 
