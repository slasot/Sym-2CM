...
\item A path $\rho$ is said to be \emph{fireable} at a configuration $q(\vr v)$
    ...
    This amounts to requiring that $\rho$ can be executed starting from $q(\vr v)$,
    without the counters having to go negative along the way.
...
We define the \emph{norm} of a vector $\vr w = (w_1,w_2) \in \Z^2$ 
to be the maximal absolute value of its components,
$\norm{\vr w}=\max\{|w_1|,|w_2|\}$, and 
the norm of a 2-\vass $\V=(Q,T)$ to be the maximal norm of the effects 
of its transitions, 
$\norm \V = \max\{\norm{\vr w} \mid (q,\updinstr{\vr w},q')\in T\}$.
...
$\gamma \in \N$ and some particular $D_\sigma$, which depends on both $\V$ and $\sigma$.
...
and both $\ell$ and $e$ are bounded by a polynomial of the the size
$
 \mysize \V = 
\mysize Q + 
2 \cdot \norm \V \cdot \mysize {T}
% \, + \, \norm w \, + \, \norm{w'},
$
of the unary representation of $\V$.
...
After this operation, the lengths of the one-off paths are bounded by
$e' := e (2C_0 \ell + \ell + 1)$.
% \sla{changed}
% \mei{nice catch, the previous value indeed wasn't enough.}
% $e' := e(\ell+1) + 2C_0\ell$. \sla{changed to 'sum of lengths'}
...
% 
Indeed, as we are assuming that $a' - a > C$, the sum of counters
...
\[
    C \ \geq \ C_1 \ := \ 
    2\norm\V e'_\mathrm{max} (\ell_\mathrm{max} + 1),
\]
where $e'_\mathrm{max}$ and $\ell_\mathrm{max}$ are the 
(polynomial in $\mysize\V$) bounds
...
% }
% \sla{let's keep them. at least $l_{max}$ we need}
% \mei{yup, as currently written, $e'$ and $\ell$ are run-specific, and
% then we take the max over all runs}
...
We classify these into three cases (\Cref{fig:cases}, left).
...
\begin{scope}
    ...
    \fill[orange!10] (0,0) -- (0,2) -- (-2,2) -- cycle;
    \fill[orange!10] (0,0) -- (2,0) -- (2,-2) -- cycle;
    \fill[black!10] (-2,-2) -- (2,-2) -- (-2,2) -- cycle;
    % axes
    \draw[->, black!50] (-2,0) -- (2,0);
    \draw[->, black!50] (0,-2) -- (0,2);

    \draw[->, very thick, green!70!black] (0,0) -- (0,2)
        node[above, green!50!black] {\footnotesize Case 1};
    \draw[->, very thick, blue!70!black] (0,0) -- (2,0)
        node[right, blue!50!black] {\footnotesize Case 3};
    \node at (1,1) {Case 3};
    \node[rotate=-45] at (1.4,-0.6) {\footnotesize Case 2};
    \node[rotate=-45] at (-0.6,1.4) {\footnotesize Case 2};

    % boundary of sum-increasing effects
    \draw[very thick] (-2,2) -- (2,-2);

    \node[rotate=-45,align=center] at (-0.6,-0.6)
            {Not sum-increasing \\ (irrelevant)};
\end{scope}
\begin{scope}[xshift=6cm]
    \fill[cyan!10] (0,0) -- (-1,2) -- (2,2) -- (2,-1) -- cycle;
    ...
    % target direction
    \draw[->, very thick, green!60!black] (0,0) -- (0, 1.5)
        node[above right, black, pos=0.8, scale=0.8] {$(0, D_\sigma)$};

    % cycle effects
    \draw[->, very thick] (0,0) -- ($ 0.5*(-0.8, 1.6) $)
        node[above left] {$c_j$};
    \draw[->, very thick] (0,0) -- ($ 0.5*(1.6, -0.8) $)
        node[below right] {$\overline{c_j}$};

    % boundary of sum-increasing effects
    \draw[dashed] (-2,2) -- (2,-2);
    % axis of symmetry
    \draw[dashed] (-2,-2) -- (2,2);
        % node[above, pos=.9, rotate=45] {\scriptsize axis of}
        % node[below, pos=.9, rotate=45] {\scriptsize symmetry};
\end{scope}
\end{tikzpicture}
\caption{
    A view of the $(\Delta x, \Delta y)$ space of effects of individual cycles.
    \figleft An illustration of how the runs are split into
    three cases, based on the effects of the sum-increasing cycles
    present in the run.
    Cases 1--2 have \emph{priority}, in that as long as at least one
    cycle in the relevant area is present, the run is handled
    by one of these cases. On the other hand,
    Case 3 is the \emph{fallback}: it handles runs where
    \emph{all} sum-increasing cycles fall in the area labeled ``Case 3'' (blue).
    \figright Proof strategy for Case 2.
    Given a cycle $c_j$, we make use of the symmetry assumption
    to obtain the transposed cycle $\overline{c_j}$.
    The shaded area demarcates the effects we can obtain from linear
    combinations of $c_j$ and $\overline{c_j}$ with positive coefficients.
    Observe that the shaded area contains the desired vertical effect
    (green, not to scale).
}
\label{fig:cases}
\end{figure}
...
As an outline, we will seek to increase the iteration count $n_j$ of
the cycle $c_j$, and introduce into the run the transposed cycle
$\overline{c_j}$, such that the net effect of our modifications
becomes vertical (see \Cref{fig:cases}, right).
...
we have
\begin{gather*}
    y_i \,=\, y_0 + iy \,\geq\, y_0 + i \,\geq\, C_0, \\
    x_i \,=\, x_0 - ix \,=\, x_{n_j} + (n_j - i) x
    \,\geq\, x_{n_j} + n_j - i \,\geq\, C_0.
    \qedhere
\end{gather*}
%
...
we must have $n_j > 2C_0$. Thus,
the range of values of $i$ admissible for \Cref{claim:fireable}
is non-empty. Setting $i := C_0 + 1$, we have that the transposed cycle
$\overline{c_j}$ is fireable after the first $C_0 + 1$ iterations
of $c_j$ within $\sigma$. Equivalently, shifting the point of view
one iteration earlier, the concatenation $c_j; \overline{c_j}$
...
\begin{tikzpicture}[
    ...
    \begin{scope}
        \fill[green!10] (0,0) rectangle (3,3);
        \fill[green!10] (0,0) rectangle (-3,-3);
        \fill[red!10] (0,0) rectangle (-3,3);
        \fill[black!15] (0,0) rectangle (3,-3);
        \fill[black!15] (0,0) -- (0,3) -- (-3,3) -- cycle;
        \fill[red!10] (0,0) -- (0,-3) -- (3,-3) -- cycle;
      % axes
      \draw[->] (-3,0) -- (3,0) node[right] {$x$};
      \draw[->] (0,-3) -- (0,3) node[above] {$y$};

      % rays corresponding to steepness
      ...
      % \node[above left] at (-0.2,2.09) {vertical effect};
        ...
        % sign labels
        % \node[scale=0.8,below left] at (3,0) {$x_2>0$};
        % \node[scale=0.8,below right] at (-3,0) {$x_1,x_3<0$};

        \node[scale=0.8,above right] at (-3,-3) {$\varphi > 0$};
        \node[scale=0.8,below left] at (3,3) {$\varphi > 0$};
        \node[scale=0.8,above right] at (0,-3) {$\varphi < 0$};
        \node[scale=0.8,above right] at (-3,0) {$\varphi < 0$};
        \node[scale=0.7,below left,align=right] at (0,3) {not in \\ Case 3};
        \node[scale=0.7,below left,align=right] at (3,0) {not in \\ Case 3};
    \end{scope}
    ...
    \begin{scope}[xshift=8cm]
        ...
        \draw[->, thick, dotted, green!50!black] (a) -- (0, 2.0)
            node[left] {$\checkmark c_{j_1}$};
        \draw[->, thick, dotted, green!50!black] (a) -- (0, 1.0)
            node[left] {$\checkmark c_{j_2}$};
        \draw[->, thick, dotted, red!50!black] (a) -- (0, -1.5)
...
\caption{
  ...
  \figleft An illustration of the properties of steepness.
  ...
  The area shaded gray corresponds to sum-increasing cycles that get handled in Case 2,
  and thus cannot occur in Case 3.
  \figright A sum-increasing cycle $c_i$,
  with three candidates for a subsequent horizontally-negative cycle $c_j$ (cf. \Cref{claim:steep}),
  illustrated in terms of their combined effect.
...
\coordinate (P0) at (0.0,1.5);
\coordinate (P1) at ($ (P0) + (0.5,-0.6) $);
\coordinate (P1e) at ($ (P1) + (1.7,0.2) $);
\coordinate (P2) at ($ (P1e) + (0.5,0.8) $);
\coordinate (P2e) at ($ (P2) + (2.3,2.3) $);
\coordinate (P3) at ($ (P2e) + (1.0,0.5) $);
\coordinate (P3e) at ($ (P3) + (-0.8,-3) $);
\coordinate (P4) at ($ (P3e) + (1.0,0.2) $);
\coordinate (P4e) at ($ (P4) + (1.5,1.0) $);
\coordinate (P5) at ($ (P4e) + (0.5,1.0) $);
\coordinate (target) at (0,4);

% Ordinary path segments
\draw[dotted,->] (P0) -- (P1);
\draw[->,very thick,green!70!black] (P1) -- (P1e) node[pos=.55,above,black] {$c_1^{n_1}$};
\draw[dotted,->] (P1e) -- (P2);
\draw[->,very thick,green!70!black] (P2) -- (P2e) node[pos=.45,right,black] {$c_2^{n_2}$};
\draw[dotted,->] (P2e) -- (P3);
\draw[->,red!70!black] (P3) -- (P3e) node[pos=.55,right,black] {$c_3^{n_3}$};
\draw[dotted,->] (P3e) -- (P4);
\draw[->,green!70!black] (P4) -- (P4e) node[pos=.45,above,black] {$c_4^{n_4}$};
\draw[dotted,->] (P4e) -- (P5);
...
\fill (P4e) circle (1.5pt);
\fill (P5) circle (1.5pt);
\fill (target) circle (1.5pt);

\node[scale=0.7,left] at (P0) {$(0, a)$};
\node[scale=0.7,left] at (target) {$(0, a')$};
\node[scale=0.7,below right]      at (P1) {$(X^\pre_1, Y^\pre_1)$};
\node[scale=0.7,below right]      at (P1e) {$(X^\post_1, Y^\post_1)$};
\node[scale=0.7,below right]      at (P2) {$(X^\pre_2, Y^\pre_2)$};
\node[scale=0.7,above left]      at (P2e) {$(X^\post_2, Y^\post_2)$};
\node[scale=0.7,right]      at (P3) {$(X^\pre_3, Y^\pre_3)$};
\node[scale=0.7,below] at (P3e) {$(X^\post_3, Y^\post_3)$};
\node[scale=0.7,below right]      at (P4) {$(X^\pre_4, Y^\pre_4)$};
\node[scale=0.7,below right] at (P4e) {$(X^\post_4, Y^\post_4)$};
\node[scale=0.7,right]      at (P5) {$(X^\pre_5, Y^\pre_5)$};

  % angle / steepness indication
  % \node at (11,5.0) {$\varphi_1<\varphi_2<\varphi_3$};

% invariant lines
\draw[dashed,thick,blue!70!black] ($ (0,2) + -0.2*(1.7,0.2) $) -- +($ 1.7*(1.7,0.2) $);
\draw[dashed,thick,blue!70!black] ($ (0,2.5) + -0.2*(1,1) $) -- +($ 2.7*(1,1) $);
\end{tikzpicture}

\caption{A run prefix that does not yet satisfy \Cref{claim:steep}.
Solid lines represent iterated cycles, and dotted lines represent 
one-off paths. Record-breaking cycles are drawn using thicker lines;
the color of the cycle indicates whether the cycle increases
or decreases the counter sum.
Parallel to each record-breaking cycle, dashed lines represent
the successive invariant lines, below which the configuration
will stay.
Note the target configuration $(0,a')$, above both invariant lines.
...
(see \Cref{fig:steep-counterexample}).
We will then show that the supposed target configuration of the run
is \emph{above} all the invariant lines, to derive a contradiction.

For a cycle $c_i$, denote by $\beta_i$ the steepness of
...
\end{equation}
Geometrically, this defines the dashed lines in \Cref{fig:steep-counterexample}.
...
\begin{tikzpicture}[
    ...
    \begin{scope}
        ...
        \draw[dashed,thick,blue!70!black] ($ (0,1.5) + -0.2*(1,0.3) $) -- +($ 3.5*(1,0.3) $);

        \coordinate (conf) at (1.7,1.7);
        \fill (conf) circle (1.5pt);
        \draw[->, green!70!black] (conf) -- +($ 1.5*(1,0.3) $);
        \draw[->, green!70!black] (conf) -- +($ 0.6*(1.3,-2) $);
        \draw[->, red!70!black] (conf) -- +($ -1.5*(1,0.3) $);
        \draw[->, red!70!black] (conf) -- +($ 0.6*(-1,-2) $);
        \draw[->, dotted, thick] (conf) -- +($ 1.5*(-1,0.3) $);
    \end{scope}
    ...
    \begin{scope}[xshift=5cm]
        ...
        \coordinate (conf) at (1.5,1.5);
        \coordinate (slope) at (0.5,0.9);
        \coordinate (shift) at (-0.5,0);
        \draw[dashed,thick,blue!70!black] ($ (conf) + -1.5*(slope) $) -- +($ 2.5*(slope) $);
        \draw[dashed,thick,blue!70!black] ($ (conf) + (shift) + -0.5*(slope) $) -- +($ 2.5*(slope) $);
        \draw[->, dotted] (conf) -- +(shift);
        \draw[->, dotted] (conf) -- +(0,0.9);
        \fill (conf) circle (1.5pt);
    \end{scope}

    \begin{scope}[xshift=10cm]
        \coordinate (root) at (0,1);
        \coordinate (conf) at (1.5,0.8);
        \coordinate (slope1) at (1,0.3);
        \coordinate (slope2) at (1,0.7);

        \fill[black!10] (0,0) -- (root) -- ($ (root) + 3*(slope1) $) -- (3,0) -- cycle;

        \draw[->] (0,0) -- (3,0) node[right] {$x$};
        \draw[->] (0,0) -- (0,3) node[above] {$y$};

        \draw[dashed,thick,blue!70!black] ($ (root) + -0.4*(slope1) $) -- +($ 3.5*(slope1) $);
        \draw[dashed,thick,blue!70!black] ($ (root) + -0.4*(slope2) $) -- +($ 3.1*(slope2) $);
        \draw[->,dotted] ($ (root) + 3*(slope1) $) arc (16.7:35:3cm);
        % \fill (conf) circle (1.5pt);
    \end{scope}
\end{tikzpicture}
\caption{
    A geometric intuition for why the inductive invariant gets preserved.
    \figleft Horizontally positive cycles (green) stay below the invariant line,
    as by construction they are at most as steep as the invariant's slope;
    horizontally negative cycles either stay below the invariant line (red)
    or satisfy \Cref{claim:steep} (dotted black).
    \figcenter One-off paths may force the invariant line to be raised,
    but this can be bounded ahead of time. 
    % The greatest upwards
    % movement per unit effect is forced by a path %whose effect is pointing
    % going directly left, when the slope of the invariant line is high.
    \figright Because the configuration is guaranteed to stay at $x \geq 0$,
    we may freely make the invariant line steeper as long as the $y$-intercept
    doesn't get lowered in the process.
}
\label{fig:induction}
\end{figure}
We shall follow the structure of the run, aiming first to prove
that after executing the effect of $c_i$, the configuration
$(X^\post_i, Y^\post_i)$ stays below the same line, \ie
\begin{equation} \label{eq:steep-ind-post}
    Y^\post_i \, \leq \, Y^\pre_k \, + \, S_i (1 + \beta_i) \, + \, \beta_i X^\post_i.
\end{equation}
See \Cref{fig:induction} (left) for an illustration of this step.
Symbolically, writing $X^\post_i = X^\pre_i + n_i x_i$ and $Y^\post_i = Y^\pre_i + n_i y_i$, we see that
...
Our next focus shall be handling the effect of $t_i$ (\Cref{fig:induction}, center), proving that
...
Having now established \eqref{eq:steep-ind-pre1}, it
remains to raise the slope of the invariant line to $\beta_{i+1}$
(compare \Cref{fig:induction}, right).
We note that
...
To summarize, in all the three cases 
...
and one \emph{auxiliary counter}, which is always zero outside of
auxiliary states.
...
we include in $T'$ the transitions in use by the following fragment of a run:
\[ \begin{split}
    q&(3a + 2, && 3b + 2, && 0)
...
We now need to verify that the cyclic shifts of these transitions
do not cause any issues.

Let us first consider the $\updinstr{3x-1, 3y-1, 0}$ instruction.
...
before the transition, they can only have a remainder of $1$ or $2$ after
any cyclic rotation executes. In particular, none of the main counters may
become zero.
Hence, should any unwanted cyclic rotation of the first update instruction execute,
...
We now turn to simulating the zero-test transitions of $\mach$.
