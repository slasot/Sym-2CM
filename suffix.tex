...
(see \Cref{fig:steep} for illustration).

\mei{TODO: Figure: demonstrate some non-trivial steepness comparisons; 
illustrate a run that
doesn't satisfy the claim below to show what we want to rule out.}
\sla{I did sth}
\mei{I modified the caption on the counterexample figure to make the point
I wanted to make, I'll adjust the other figure as well later.}
...
\begin{tikzpicture}[
  ...
  \draw[dashed] (0,0) -- (2.4,1.2)
      node[pos=.92, below] {$\varphi_2$};

  \draw[dashed] (0,0) -- (-2.5,1.3)
      node[pos=.75, above] {$\varphi_3$};

  % cycle effects
  \draw[->, very thick] (0,0) -- (-1.3,-1.5)
      node[pos=.62, right] {$c_1$};

  \draw[->, very thick] (0,0) -- (1.7,0.88)
  ...
  % sign labels
  \node at (2.1,-0.3) {$x_2>0$};
  \node at (-2.0,-0.3) {$x_1,x_3<0$};
\end{tikzpicture}
\caption{
  The steepness comparison used in
  \Cref{claim:steep}.
...
\caption{A run violating \Cref{claim:steep}.
Thick lines represent iterated major cycles, and dotted lines represent 
one-off paths.
The cycles $c_1$ and $c_2$ are 
(horizontally positive) major and sum-increasing, the latter steeper than
the former, and $c_3$ is a horizontally negative major (sum-decreasing) cycle
steeper than $c_1$ and $c_2$.
% Thus none of major sum-increasing cycles is followed by a less steep
% horizontally negative cycle.
Observe how this run gets further away from the Y axis as it progresses.
Intuitively, runs violating \Cref{claim:steep} are ruled out, because
we're considering runs that end \emph{on} the Y axis,
above the starting point of the run.
To extend the run and have it reach such an endpoint, one would need to either add
a cycle steeper than $c_2$, or add a horizontally-negative sum-increasing cycle,
putting the run in the jurisdiction of Case 2.}
\label{fig:claim13-counterexample}
\end{figure}
...
Suppose now that the claim doesn't hold
(see \Cref{fig:claim13-counterexample} for illustration).
Then, for any horizontally negative cycle $c_j$,
Let $c_{k_1}, \ldots c_{k_s}$ be the sequence
...
At the start of the run, set $S_0 = 0$, $\beta_0 = -1$, and 
$(X_0, Y_0) = (0, a)$.
