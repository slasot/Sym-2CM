...
\begin{equation} \label{eq:steep-interm}
    \begin{split}
        ...
        & \mei{\not \leq a \, + \, \beta_{i+1} (x-S^x_{i+1})}
    \end{split}
\end{equation}
\mei{($x - S^x_{i+1}$ can be negative!)}
...
\[
    ...
    \end{split}
\]
\sla{new proposed version below}
\[
    \begin{split}
    Y_{i+1} - S^y_{i+1} \ =\ y \,+\, n_{i+1} y_{i+1}  - S^y_{i+1} 
    &\ \leq\ a \,+\, \beta_{i+1} (x + n_{i+1} x_{i+1} - S^x_{i+1}) \\
    &\ =\  a \,+\, \beta_{i+1} (X_{i+1}-S^x_{i+1}),\\
    \end{split}
\]
